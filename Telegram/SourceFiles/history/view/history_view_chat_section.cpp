...
void ChatMemento::setFromTopic(not_null<Data::ForumTopic*> topic) {
	...
	if (!_list.aroundPosition()) {
		...
		_listFromSharedState = true;
	}
}

void ChatMemento::discardSharedListState() {
	if (_listFromSharedState) {
		_list = ListMemento();
		_listFromSharedState = false;
	}
}
...
void ChatWidget::restoreState(not_null<ChatMemento*> memento) {
	if (controller()->windowId().type == Window::SeparateType::Chat) {
		memento->discardSharedListState();
	}
...
bool ChatWidget::listIsMainView() {
